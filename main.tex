...
To systematically resolve the QoS Illusion, tele-radiology networks must move beyond monolithic pixel-distortion metrics. We propose a full-stack, 4-layer diagnostic-aware communication architecture spanning source perception, compression, radio access, and transport semantics (Fig.~\ref{fig:architecture} and Table~\ref{tab:comparison}).

\subsection{Architectural Overview of the 4-Layer Stack}
The proposed framework decouples diagnostic integrity from gross anatomical transmission across four coordinated layers: (1)~\textbf{Layer 1 (Perception \& Audit):} embeds the Causal Safety Audit (CSA) directly into transmission pre-checks, replacing uniform pixel distortion with the task-based Channelised Hotelling Observer (CHO) that models human visual neuro-radiology; (2)~\textbf{Layer 2 (Diagnostic Compression):} replaces heuristic subsampling with Diagnostic-Aware Rate-Distortion Optimization (DARDO), allocating bits strictly proportional to diagnostic spectral density; (3)~\textbf{Layer 3 (5G RAN Slicing):} enforces differentiated QoS scheduling, routing mission-critical stroke frequencies over 5G Ultra-Reliable Low-Latency Communication (URLLC) slices while passing background anatomy over standard enhanced Mobile Broadband (eMBB); and (4)~\textbf{Layer 4 (Transport \& Semantic Verification):} deploys lightweight 128-byte semantic tokens in QUIC packet headers, closing the loop with Selective Negative Acknowledgment (S-NACK) retransmissions for missing diagnostic frequencies.
...
Acute Stroke Erasure & $\mathbf{28.79\%}$ (Clinical Hazard) & $\mathbf{< 4.5\%}$ (Clinically Safe) \\
...
\label{eq:gamma}
\end{equation}
where $\mathcal{S}_k$ represents receiver coil sensitivities; and (3)~\textbf{Greedy Line Allocation:} seed sampling mask $\mathcal{M}$ with autocalibration center lines and iteratively insert trajectories maximizing $\gamma_k$ until $d'_{\text{CHO}}(\mathcal{M}) \ge 1.5$. Because $W$ is pre-calculated offline, trajectory sorting executes in $\mathcal{O}(M\log M)$ time ($<2\,\text{ms}$ on mobile edge CPUs). As summarized in Table~\ref{tab:comparison}, under identical $R=8$ payload ($87.5\%$ bandwidth savings), DARDO slashes acute stroke silent erasure from $28.79\%$ down to $<4.5\%$.

\subsection{5G URLLC Slicing \& Prioritized Scheduling}
Mobile Stroke Units face unpredictable cellular channel conditions, including multi-path fading, shadowing, and cell handover during ambulance transit. Under standard single-carrier streaming (e.g., DASH), a dropped burst of packets is treated uniformly. If dropped packets coincide with high-frequency stroke bands, the edge AI reconstructs smooth false-normal brain tissue.

Under our Layer 3 protocol, the transmitter dynamically tags packets by diagnostic utility: (1)~\textbf{Critical Slice (5G URLLC):} packets containing high-utility lines ($\gamma_k \ge \tau$) are mapped to 5G QoS Class Identifier (QCI) 1/65 (URLLC). The cellular scheduler guarantees Block Error Rate (BLER) $<10^{-5}$ and sub-10 ms latency, shielding stroke details against deep fading dips; and (2)~\textbf{Bulk Slice (5G eMBB):} lines carrying low diagnostic density ($\gamma_k < \tau$) are routed over best-effort eMBB slices, absorbing transient network congestion without threatening diagnostic integrity.

\begin{figure*}[t]
\centering
\includegraphics[width=0.96\textwidth]{figures/fig4_architecture.pdf}
...
In 6G semantic communications \cite{zhang2022semantic}, decoders verify diagnostic meaning rather than syntactic bit identity. The mobile ambulance AI embeds a compact 128-byte semantic pathology token (Gabor filter responses $\mathbf{v}$ and ROI coordinates) into the QUIC/RTP transport header. Upon edge reconstruction via VarNet \cite{sriram2020endtoend}, the hospital server evaluates the model observer in $<15\,\text{ms}$. If detectability satisfies $z_{\text{recon}} \ge 1.5$, the scan is verified. If $z_{\text{recon}} < 1.5$, the transport layer issues a targeted \textbf{Selective Negative Acknowledgment (S-NACK)}. Rather than retransmitting the full raw volume ($>200\,$MB) and violating triage deadlines, the S-NACK requests only the missing high-$\gamma_k$ trajectories ($\Delta k \approx 1.2\,\text{MB}$), restoring detectability over the URLLC slice in $\approx 15\,\text{ms}$.
